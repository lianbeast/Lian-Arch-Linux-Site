...
export default function PacmanGame({ onClose }) {
  ...
  const initGame = useCallback(() => {
    ...
    setScore(0); setLives(3); setGameState('ready')
    // Persist high score across restarts (already in st from prior game)
  }, [])
  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      // ── Update ──
      if (st.readyTimer > 0) { st.readyTimer--; render(ctx, st); raf = requestAnimationFrame(loop); return }
      if (gameState === 'ready') setGameState('playing')

      if (gameState === 'dying') {
        st.dyingTimer++
        if (st.dyingTimer > 60) {
          if (st.lives <= 0) { setGameState('over'); render(ctx, st); raf = requestAnimationFrame(loop); return }
          resetPositions(st)
          setGameState('ready'); st.readyTimer = 40; st.dyingTimer = 0
          render(ctx, st); raf = requestAnimationFrame(loop); return
        }
        render(ctx, st); raf = requestAnimationFrame(loop); return
      }
      ...
      st.ghosts.forEach(g => {
        ...
        if (dx < TILE * 0.7 && dy < TILE * 0.7) {
          if (g.frightened) {
            ...
            st.score += 200; setScore(st.score)
            sndEatGhost()
          } else {
  ...
  return (
    <div className="pacman-overlay">
      <div className="pacman-container">
        <canvas ref={canvasRef} width={W} height={H} className="pacman-canvas" />
      </div>
      <button className="pacman-close" onClick={onClose} aria-label="Close game">✕</button>
    </div>
  )
}
